...
from specfempp import set_par, load_par, get_par, execute, del_par
  
# %%
# So that we don't start completely from scratch, let's load a parameter file

load_par('specfem_config.yml')

# %% 
# The parameters can be inspected using the `get_par` function.

print(get_par('header.description'))
print("DT:", get_par('simulation-setup.solver.time-marching.time-scheme.dt'))
print("NT:", get_par('simulation-setup.solver.time-marching.time-scheme.nstep'))

# %%
# Let's set the number of time steps to 1000 and the time step size to 0.002.
set_par('simulation-setup.solver.time-marching.time-scheme.dt', 0.002)
set_par('simulation-setup.solver.time-marching.time-scheme.nstep', 5000)
...
print(get_par('databases.source-file'))
