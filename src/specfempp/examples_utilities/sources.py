...
def create_sources(angle, vp: float, 
                   ...
                   iz: float = 720.0, dz: float = 100.0, fz: float = 20000.0,
    ...
    for i in range(197):
        ...
        moment_tensor["moment-tensor"]["x"] = x
        moment_tensor["moment-tensor"]["z"] = iz
        ...
        moment_tensor["moment-tensor"]["Ricker"]["factor"] = 1e-9 * np.cos(angle * np.pi / 180)
        moment_tensor["moment-tensor"]["Ricker"]["tshift"] = tshift
    ...
    if angle >= 5:
        ...
        for ii in range(50):
            ...
            moment_tensor = {"moment-tensor": dict()}
            moment_tensor["moment-tensor"]["x"] = ix
            moment_tensor["moment-tensor"]["z"] = z
            moment_tensor["moment-tensor"]["Mxx"] = 1.0
            moment_tensor["moment-tensor"]["Mzz"] = 1.0
            moment_tensor["moment-tensor"]["Mxz"] = 0.0
            moment_tensor["moment-tensor"]["angle"] = 0.0
            moment_tensor["moment-tensor"]["Ricker"] = dict()
            moment_tensor["moment-tensor"]["Ricker"]["factor"] = 1e-10
            moment_tensor["moment-tensor"]["Ricker"]["tshift"] = tshift
            moment_tensor["moment-tensor"]["Ricker"]["f0"] = 1.0
            zcount += 1
            sources.append(moment_tensor)
...
def plot_sources(sources: dict):
    ...
    # plot sources
    x = [source['moment-tensor']['x'] for source in sources]
    z = [source['moment-tensor']['z'] for source in sources]
    t = [source['moment-tensor']['Ricker']['tshift'] for source in sources]
